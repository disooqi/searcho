namespace Microsoft.Samples.DirectorySearcher
{
	...
	public class DirectorySearcher : Control
	{
		...
		public System.Windows.Forms.CheckedListBox.CheckedItemCollection CheckedItems
		{
			get
			{
		...
		private void AddFiles(string[] files, int startIndex, int count)
		{
			while(count-- > 0)
			{
				CheckedListBox.Items.Add(files[startIndex + count]);
				CheckedListBox.SetItemChecked(CheckedListBox.Items.Count-1,true);
				//FilePaths.Add(files[startIndex + count]);
			}
		}

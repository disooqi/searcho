...
namespace SearchO
{
	...
	public class BuildingIndex : Control
	{
        ...
        private PDFParser pdfObj = new PDFParser();
		...
		public void Tokenization(IEnumerator IE, int count)
		{
			try
			{
				...
				while(IE.MoveNext())
				{
					try
					{
                        BeginInvoke(APD, new object[] { ((int)Math.Ceiling((decimal)100 * docID / count)) / 4, (int)Math.Ceiling((decimal)100 * docID / count), " Tokenizing .... : " + IE.Current.ToString() });
						dataAccess.givingIdForDoc(indexDirPath ,docID,IE.Current.ToString());
                        DocTokenization(IE.Current.ToString(), docID);				
						docID++;
					}
					catch(Exception ee)
					{
		...
		private void DocTokenization(string DocPath, int docID)
		{
			try
			{
